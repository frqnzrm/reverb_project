...
const juce::String receiver = "BinauralDecoder";
...
struct JackPortIterator
{
    ...
    bool next()
    {
        ...
        name = juce::CharPointer_UTF8 (ports[++index]);
...
static juce::Array<JackAudioIODeviceType*> activeDeviceTypes;

//==============================================================================
class JackAudioIODevice   : public juce::AudioIODevice
{
public:
    JackAudioIODevice (const juce::String& deviceName,
          ...
          totalNumberOfOutputChannels (0)
    {
        ...
        client = jack_client_open (JUCE_JACK_CLIENT_NAME, JackNoStartServer, &status);
    ...
    juce::StringArray getChannelNames (bool forInput) const
    {
    ...
    juce::StringArray getOutputChannelNames() override         { return getChannelNames (false); }
    juce::StringArray getInputChannelNames() override          { return getChannelNames (true); }
    ...
    int getCurrentBufferSizeSamples() override      { return client != nullptr ? static_cast<int> (jack_get_buffer_size (client)) : 0; }
    double getCurrentSampleRate() override          { return client != nullptr ? static_cast<int> (jack_get_buffer_size (client)) : 0; }
                 ...
                 double /* sampleRate */, int /* bufferSizeSamples */) override
    {
        ...
        {
            juce::String inputName;
            inputName << receiver << ":in_" << i+1;
            jack_connect (client, jack_port_name(outputPorts[i]), inputName.toUTF8());
    ...
    int getOutputLatencyInSamples() override
    {
        ...
        for (int i = 0; i < outputPorts.size(); i++)
            latency = juce::jmax (latency, (int) jack_port_get_total_latency (client, (jack_port_t*) outputPorts [i]));
    ...
    int getInputLatencyInSamples() override
    {
        ...
        for (int i = 0; i < inputPorts.size(); i++)
            latency = juce::jmax (latency, (int) jack_port_get_total_latency (client, (jack_port_t*) inputPorts [i]));
...
private:
    void process (const int numSamples)
    {
        ...
        else
        {
            for (int i = 0; i < numActiveOutChans; ++i)
                juce::zeromem (outChans[i], sizeof (float) * numSamples);
        }
    }
    ...
    juce::Array<const jack_port_t *> inputPorts, outputPorts;
...
private:
    juce::StringArray inputNames, outputNames, inputIds, outputIds;
    bool hasScanned;
